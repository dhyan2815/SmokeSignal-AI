...
- **Branch:** Created the `legacy` branch to isolate this restoration while preserving the `main` branch.
- **Memory Preservation:** Restored `GEMINI.md` to ensure project context is maintained despite the rollback.

### [2026-04-22] Git Configuration & Cleanup
- **Git Cleanup:** Removed tracked `__pycache__` directories and `.pyc` files from the Git index using `git rm --cached`. This ensures that local cache files generated by the backend server no longer appear as changes in Git.
- **Gitignore Expansion:** Significantly expanded the root `.gitignore` file with comprehensive patterns for Python, Node.js, IDEs, and OS-specific files to maintain a clean repository.
